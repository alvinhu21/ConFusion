...
angular.module('confusionApp')
		...
		.controller('FeedbackController', ['$scope', 'feedbackFactory', function($scope,feedbackFactory){
			...
			$scope.hasProfanity = false;
			$scope.checkProfanity = function(){
				$scope.hasProfanity = false;
				var word;
				for(word in $scope.feedback.comments.toUpperCase().split(" ")){
					
					if(word.includes("FUCK")|| (word == "SHIT") || (word == "ASS") || word.includes("ASSHOLE") || word == "ASSES"
					|| word.includes("BITCH") || word.includes("CUNT") || word.includes("COCKSUCK")){
						$scope.hasProfanity = true;
						break;
					}
				}
        ...
        .controller('DishDetailController', ['$scope', '$stateParams', 'menuFactory', function($scope, $stateParams, menuFactory) {
			...
			$scope.DishType = "mains";
            $scope.message="Loading ...";
			$scope.dishes = menuFactory.getDishes().query().$promise.then(
				function(response){
					$scope.dishes = response;
					//$scope.dish = $scope.dishes[0];
					$scope.showDish = true;
					$scope.dishes.forEach(function(e){
						e.showComments = false;
					});
				},
				function(response) {
					$scope.message = "Error: "+response.status + " " + response.statusText;
